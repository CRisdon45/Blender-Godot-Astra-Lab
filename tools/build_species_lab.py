"""Blender adapter: two species, 3 illustrative stages, 2 seeds, 3 LODs.
No baseline/courtyard files are read or modified. Original atlases; no third-party assets.
"""
from __future__ import annotations
import dataclasses, hashlib, json, math, sys
...
def atlas(species:str, flower:bool=False):
    ...
    for k in range(4):
        ...
        if flower:
            # Three small blossom marks instead of a single oversized star.
            for cx,cy in ((.34,.36),(.63,.45),(.45,.67)):
                dx,dy=xx-cx,yy-cy
        ...
        eroded=mask.copy()
        for axis in (0,1): eroded &= np.roll(mask,1,axis)&np.roll(mask,-1,axis)
...
def make_cards(name,cards,lod,mat,recipe):
    ...
    for c in cards:
        ...
        for x,z in ((0,0),(1,0),(1,1),(0,1)):
            ...
            colors.append((c.tile/3,c.shade,c.rank,1));preview.append(((c.tile+x)/4,z))
        faces.extend([(offset,offset+1,offset+2),(offset,offset+2,offset+3)])
    obj=make_mesh(name,verts,faces,mat);data=obj.data
    # Allocate layers before obtaining RNA handles. Runtime mats use no textures,
    # preserving UVMap/BrushSize as TEXCOORD_0/TEXCOORD_1 in the exported GLB.
...
manifest={'schema':'species-witness/1','generator_version':'0.2.0','units':'metres',
...
for species,profile in PROFILES.items():
    ...
    for seed in (41,73):
        for stage,maturity in enumerate((0,.5,1)):
            ...
            (OUT/(key+'.json')).write_text(json.dumps(dataclasses.asdict(plant),separators=(',',':'))+'\n')
            for lod in range(3):
                ...
                report.update({'species':species,'seed':seed,'stage':stage,'lod':lod,'file':target.name,'sha256':hashlib.sha256(target.read_bytes()).hexdigest()})
