...
def compile_plant(species:str, seed:int=41, maturity:float=1.0) -> Plant:
    ...
    for i in range(count):
        ...
        b=Branch(f'leader:{i}','root',attach,[start,c1,c2,tip],trunk.radius*(.49 if tree else .53),1)
        plant.branches.append(b)
        if tree:
            # Increasing secondary branching changes architecture, not just uniform scale.
            for j,t in enumerate((.45,.65,.83)):
                if j==0 and maturity<.18: continue
                rr=rng_for(seed,f'secondary:{i}:{j}')
                ang=a+(-1 if j%2 else 1)*rr.uniform(.50,1.0)
                radial2=w*rr.uniform(.30,.38)
                end=(math.cos(ang)*radial2,math.sin(ang)*radial2,h*rr.uniform(.58,.82))
                origin=bezier(b.points,t)
                child=Branch(f'secondary:{i}:{j}',b.id,t,[origin,add(origin,(0,0,h*.11)),mix(origin,end,.73),end],b.radius*.45,2)
                plant.branches.append(child)
                plant.lobes.append(Lobe(child.id,child.id,end,(w*.115,w*.105,h*.115)))
            plant.lobes.append(Lobe(b.id,b.id,tip,(w*.125,w*.12,h*.115)))
        else:
            plant.lobes.append(Lobe(b.id,b.id,tip,(w*.24,w*.23,h*.24)))
            for j,t in enumerate((.50,.76)):
                ...
                child=Branch(f'twig:{i}:{j}',b.id,t,[origin,mix(origin,end,.28),mix(origin,end,.72),end],b.radius*.39,2)
                plant.branches.append(child)
    # Z-up coordinate bounds and explicit supporting axes remain in the descriptor.
    total_per_lobe=64 if tree else 52
    max_brush=(.45+.23*maturity) if tree else (.14+.09*maturity)
    for li,l in enumerate(plant.lobes):
        ...
        for j in range(total_per_lobe):
            ...
            # Reject deep interior brushes covered by another lobe. Reduces overdraw,
            # without throwing away the outer three-dimensional shell.
            ...
            normal=unit(add(mul(local,.54),mul(overall,.46)))
            ...
            # A distinct flower layer rooted at existing branch-supported foliage sockets.
            if j%3==0 and z>-.55:
...
def cards_for_lod(cards:list[Card],lod:int) -> list[Card]:
    ...
    # Persistent IDs and ranked subsets. Preserve at least one card in each lobe.
    stride=(1,2,4)[lod]
    by_lobe={}
...
def wood_mesh(plant:Plant,lod:int):
    vertices=[]; faces=[]
    sides=(7,5,4)[lod]; segments=(8,5,3)[lod]
...
def metrics(plant:Plant,lod:int) -> dict:
    ...
    # Conservative camera-facing bound, deliberately larger than the physical crown.
