...
extends RefCounted
## Validated immutable-by-convention catalog. Generated by compile_plant_catalog.py.
...
func open_catalog(path: String = "res://engine_data/catalog.json") -> bool:
	...
	if file == null:
		return _fail("Plant catalog missing; run tools/compile_plant_catalog.py")
	...
	if decoded.get("schema") != "plant-catalog/1" or decoded.get("render_coordinates") != "y_up":
	...
	for value in decoded.variants:
		...
		for index in range(3):
			...
			if index > 0:
				for component in ["wood", "leaf", "flower", "total"]:
...
func _valid_costs(value: Variant) -> bool:
	if not value is Dictionary:
		return false
	var total: int = 0
	for key in ["wood", "leaf", "flower", "total"]:
