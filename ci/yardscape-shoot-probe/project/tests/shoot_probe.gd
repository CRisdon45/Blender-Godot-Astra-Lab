...
func grab(name: String) -> Image:
	...
	return image
func set_isolated(value: bool) -> Array:
	var saved: Array=[]
	for n in study.derived.get_children():
		saved.append([n,n.visible])
		n.visible = (n == study.compact_shoot) if value else n.visible
	return saved
...
func all_finite_and_bounded() -> bool:
	var t: Dictionary=study.document.tree
	var root: Vector3=Vector3(t.x,t.base_elevation,-t.y)
	for node in [study.compact_shoot.twig,study.compact_shoot.foliage]:
		...
		for p in a[Mesh.ARRAY_VERTEX]:
			...
			if Vector2(world.x-root.x,world.z-root.z).length()>t.crown_radius*1.01: return false
	...
	return true
func geometry_hash() -> String: return study.compact_shoot.geometry_signature()
...
func run() -> void:
	...
	var initial_geometry:=geometry_hash()
	...
	diagnostics["stats"]=study.compact_shoot.stats
	diagnostics["source_tree_hash"]=source_hash
	var saved:=set_isolated(true)
	isolated_camera("top");var top:=await grab("01-isolated-top")
	...
	study._choose("Afternoon");var afternoon:=await grab("04-isolated-afternoon")
	check(digest(afternoon)!=digest(reverse),"same shoot responds to changed scene light")
	study._choose("Morning")
	check(digest(reverse)==digest(await grab("05-isolated-light-return")),"morning light return restores isolated reverse view")
	restore(saved)
	study._choose("Plan");var plan_shoot:=await grab("06-courtyard-plan-shoot")
	study.set_shoot_enabled(false);var plan_proxy:=await grab("07-courtyard-plan-proxy")
	check(digest(plan_shoot)!=digest(plan_proxy),"T comparison changes only tree representation in Plan context")
	study.set_shoot_enabled(true)
	study._choose("Orbit");study._choose("Perspective");var orbit_shoot:=await grab("08-courtyard-orbit-shoot")
	study.set_shoot_enabled(false);var orbit_proxy:=await grab("09-courtyard-orbit-proxy")
	check(digest(orbit_shoot)!=digest(orbit_proxy),"T comparison changes tree representation in Perspective context")
	study.set_shoot_enabled(true)
	var before_material:=digest(orbit_shoot)
	study.set_materials_enabled(false);await grab("10-courtyard-materials-off")
	study.set_materials_enabled(true)
	check(before_material==digest(await grab("11-materials-return")),"retained water/paving material toggle returns exactly")
	check(geometry_hash()==initial_geometry,"view light and material changes never regenerate shoot geometry")
	check(JSON.stringify(study.document.tree,"",true,true).sha256_text()==source_hash,"source tree record stays unchanged")
	var report: Dictionary={"run_id":run_id,"passed":failures.is_empty(),"checks":checks,"failures":failures,"pixel_hashes":hashes,"diagnostics":diagnostics,"engine":Engine.get_version_info().string,"adapter":RenderingServer.get_video_adapter_name(),"geometry_hash":initial_geometry,"scope":"one compact generic 3D foliage shoot, isolated and at one retained courtyard anchor","artistic_acceptance":"not_evaluated","tablet_performance_tested":false,"whole_tree_tested":false}
