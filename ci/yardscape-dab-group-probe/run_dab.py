...
def run():
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'dab-group';shutil.copytree(SPATIAL/'project',stage)
    for b in (GROUP,ROOT):
        for p in (b/'project').rglob('*'):
            if p.is_file():
                d=stage/p.relative_to(b/'project');d.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(p,d)
    shutil.copyfile(ROOT/'dab_probe.gd',stage/'tests/dab_probe.gd');(stage/'project.godot').write_text('config_version=5\n[application]\nconfig/name="Dab foliage group comparison"\n[display]\nwindow/size/viewport_width=1280\nwindow/size/viewport_height=900\n[rendering]\nrenderer/rendering_method="gl_compatibility"\n')
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'dab-group-evidence';out.mkdir(exist_ok=False);rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_DAB_OUTPUT=str(inside),YARDSCAPE_DAB_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/dab_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/dab_probe.gd'])]:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Dab-group source boundary verified')
    else:run()
