...
const DabGroup = preload("res://presentation/northstar/foliage/dab_group.gd")
var group: Node3D
func _build() -> void:
	super._build()
	for child in derived.get_children():
		if child.name=="tree-trunk" or str(child.name).begins_with("crown-"):
			child.visible=false
	group=DabGroup.new();group.name="DabFoliageGroup"
		...
		if layer is CanvasLayer:
			for child in layer.get_children():
				if child is Label and child.text.begins_with("MATERIAL TRANSFER"):
					child.text="DAB FOLIAGE GROUP / small opaque 3D paint marks / generic form test"
