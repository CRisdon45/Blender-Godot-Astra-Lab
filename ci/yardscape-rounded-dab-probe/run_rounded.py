...
def verify():
    spec=importlib.util.spec_from_file_location('dab_verify',DAB/'run_dab.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m);m.verify()
    expected={'northstar-rounded-dab-group.tscn','presentation/northstar/spatial_rounded_dab_group_study.gd','presentation/northstar/foliage/rounded_dab_group.gd'}
    actual={p.relative_to(ROOT/'project').as_posix() for p in (ROOT/'project').rglob('*') if p.is_file()}
    if actual!=expected:raise ValueError('Unexpected rounded source expansion')
    for name in actual:
        p=ROOT/'project'/name
        ...
        for ref in re.findall(r'res://([^\s\"\')]+)',text):
            if ref.endswith(('.gd','.gdshader','.gdshaderinc','.tscn')) and not any((base/'project'/ref).is_file() for base in (ROOT,DAB,GROUP,SPATIAL)):raise ValueError('Unresolved '+ref)
    return True
def run():
    verify()
    ...
    shutil.copyfile(ROOT/'rounded_probe.gd',stage/'tests/rounded_probe.gd')
    project='''config_version=5
[application]
config/name="Rounded dab component comparison"
run/main_scene="res://northstar-rounded-dab-group.tscn"
[display]
...
'''
    ...
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'rounded-dab-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_ROUNDED_DAB_OUTPUT=str(inside),YARDSCAPE_ROUNDED_DAB_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/rounded_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/rounded_probe.gd'])]:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Rounded-dab source boundary verified')
    else:run()
