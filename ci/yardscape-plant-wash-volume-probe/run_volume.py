...
def verify():
    ...
    expected={'northstar-profile-wash-volume.tscn','presentation/northstar/spatial_profiled_wash_volume_study.gd','presentation/northstar/foliage/batched_wash_volume.gdshader','presentation/northstar/foliage/profiled_batched_wash_tree.gd'}
    actual={p.relative_to(ROOT/'project').as_posix() for p in (ROOT/'project').rglob('*') if p.is_file()}
    if actual!=expected:raise ValueError('Unexpected wash-volume source expansion')
    for name in actual:
        p=ROOT/'project'/name
        ...
        for ref in re.findall(r'res://([^\s"\')]+)',text):
            if ref.endswith(('.gd','.gdshader','.gdshaderinc','.tscn')) and not any((base/'project'/ref).is_file() for base in (ROOT,BRUSH,PROFILE,FANTEX,BATCH,VALUE,DABTREE,SPATIAL,CANOPY,GROUP,DAB,COMPACT)):raise ValueError('Unresolved '+ref)
    return True
...
def run():
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'plant-wash-volume'
    if stage.exists():raise FileExistsError('Fresh stage required')
    shutil.copytree(SPATIAL/'project',stage)
    for base in (CANOPY,GROUP,DAB,COMPACT,DABTREE,VALUE,BATCH,FANTEX,PROFILE,BRUSH,ROOT):
    ...
    shutil.copyfile(ROOT/'volume_probe.gd',stage/'tests/volume_probe.gd')
    project='''config_version=5
[application]
config/name="Profile driven 3D wash volumes"
run/main_scene="res://northstar-profile-wash-volume.tscn"
[display]
...
'''
    ...
    out=Path(os.environ['RUNNER_TEMP'])/'plant-wash-volume-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4())
    env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))}
    env.update(YARDSCAPE_VOLUME_OUTPUT=str(inside),YARDSCAPE_VOLUME_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1')
    status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/volume_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/volume_probe.gd'])]:
        ...
        status='plant_wash_volume_passed'
    finally:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Wash-volume profile source boundary verified')
    else:run()
