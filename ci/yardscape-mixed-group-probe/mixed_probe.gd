extends SceneTree
const Current=preload("res://presentation/northstar/spatial_mixed_group_study.gd")
const Compact=preload("res://presentation/northstar/foliage/compact_group.gd")
const Mixed=preload("res://presentation/northstar/foliage/mixed_group.gd")
var study
var baseline
var output:String
var run_id:String
...
func valid(group)->bool:
	...
	return true
func run()->void:
	output=OS.get_environment("YARDSCAPE_MIXED_OUTPUT");run_id=OS.get_environment("YARDSCAPE_MIXED_RUN_ID")
	...
	root.size=Vector2i(1280,900);study=load("res://northstar-mixed-foliage-group.tscn").instantiate();root.add_child(study);for i in 3:await process_frame
	...
	check(study is Current and valid(study.group),"mixed candidate initializes with finite final-mesh normals")
	check(study.group.stats.masses==5 and study.group.stats.accents==4,"candidate has five overlapping masses and four selective accents")
	var mixed_triangles:=int(study.group.stats.foliage_triangles)+int(study.group.stats.twig_triangles);var compact_triangles:=int(baseline.stats.foliage_triangles)+int(baseline.stats.twig_triangles)
	check(mixed_triangles<compact_triangles,"mixed group uses fewer indexed triangles than compact literal control")
	check(not study.group.stats.alpha_blended,"candidate stays opaque")
	var signature:String=study.group.geometry_signature();var clone:=Mixed.new();clone.configure(int(t.seed),t.crown_radius*.54,t.height*.30);check(clone.geometry_signature()==signature,"candidate is deterministic");clone.free()
	set_view(.55,.34,4.4);await capture("01-front-compact",false);var front:=await capture("02-front-mixed",true)
	set_view(.55+PI,.34,4.4);await capture("03-reverse-compact",false);await capture("04-reverse-mixed",true)
	set_view(.55+PI*.5,.46,4.4);await capture("05-quarter-compact",false);await capture("06-quarter-mixed",true)
