...
def verify():
    ...
    for name in actual:
        ...
        for ref in re.findall(r'res://([^\s\"\')]+)',text):
            if ref.endswith(('.gd','.gdshader','.gdshaderinc','.tscn')) and not any((base/'project'/ref).is_file() for base in (ROOT,FANTEX,BATCH,VALUE,DABTREE,SPATIAL,CANOPY,GROUP,DAB,COMPACT)):raise ValueError('Unresolved '+ref)
    return True
def run():
    verify()
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'plant-form-profile'
    if stage.exists():raise FileExistsError('Fresh stage required')
    shutil.copytree(SPATIAL/'project',stage)
    for base in (CANOPY,GROUP,DAB,COMPACT,DABTREE,VALUE,BATCH,FANTEX,ROOT):
    ...
    shutil.copyfile(ROOT/'profile_probe.gd',stage/'tests/profile_probe.gd')
    project='''config_version=5
[application]
config/name="Plant form profile equivalence"
run/main_scene="res://northstar-profiled-fantex-ash.tscn"
[display]
...
'''
    ...
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'plant-form-profile-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_PROFILE_OUTPUT=str(inside),YARDSCAPE_PROFILE_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/profile_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/profile_probe.gd'])]:
        ...
        if len(list(inside.glob('*.png')))!=10:raise ValueError('Expected ten images')
        status='plant_form_profile_passed'
    finally:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Plant-form profile source boundary verified')
    else:run()
