...
def verify():
    spec=importlib.util.spec_from_file_location('dabtree_verify',DABTREE/'run_dab_tree.py')
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);module.verify()
    expected={'northstar-dab-value-tree.tscn','presentation/northstar/spatial_dab_value_study.gd','presentation/northstar/foliage/dab_value.gdshader'}
    actual={p.relative_to(ROOT/'project').as_posix() for p in (ROOT/'project').rglob('*') if p.is_file()}
    if actual!=expected:raise ValueError('Unexpected value-candidate source expansion')
    for name in actual:
        p=ROOT/'project'/name
        ...
        for ref in re.findall(r'res://([^\s\"\')]+)',text):
            if ref.endswith(('.gd','.gdshader','.gdshaderinc','.tscn')) and not any((base/'project'/ref).is_file() for base in (ROOT,DABTREE,SPATIAL,CANOPY,GROUP,DAB,COMPACT)):
...
def run():
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'dab-value-tree'
    if stage.exists():raise FileExistsError('Fresh stage required')
    shutil.copytree(SPATIAL/'project',stage)
    for base in (CANOPY,GROUP,DAB,COMPACT,DABTREE,ROOT):
    ...
    shutil.copyfile(ROOT/'value_probe.gd',stage/'tests/value_probe.gd')
    project='''config_version=5
[application]
config/name="Dab tree hierarchy value comparison"
run/main_scene="res://northstar-dab-value-tree.tscn"
[display]
...
'''
    ...
    out=Path(os.environ['RUNNER_TEMP'])/'dab-value-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))}
    env.update(YARDSCAPE_DAB_VALUE_OUTPUT=str(inside),YARDSCAPE_DAB_VALUE_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/value_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/value_probe.gd'])]:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Hierarchy-value candidate source boundary verified')
    else:run()
