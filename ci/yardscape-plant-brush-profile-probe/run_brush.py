...
def verify():
    ...
    expected={'northstar-profile-brush-cloud.tscn','presentation/northstar/spatial_profiled_brush_study.gd','presentation/northstar/foliage/brush_cloud_group.gd','presentation/northstar/foliage/northstar_brush_atlas.gd','presentation/northstar/foliage/batched_brush_cloud.gdshader','presentation/northstar/foliage/profiled_batched_brush_tree.gd'}
    actual={p.relative_to(ROOT/'project').as_posix() for p in (ROOT/'project').rglob('*') if p.is_file()}
    if actual!=expected:raise ValueError('Unexpected brush-profile source expansion')
    for name in actual:
        p=ROOT/'project'/name
...
def run():
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'plant-brush-profile'
    ...
    shutil.copyfile(ROOT/'brush_probe.gd',stage/'tests/brush_probe.gd')
    project='''config_version=5
[application]
config/name="Profile driven opaque brush cloud"
run/main_scene="res://northstar-profile-brush-cloud.tscn"
[display]
...
'''
    ...
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'plant-brush-profile-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_BRUSH_OUTPUT=str(inside),YARDSCAPE_BRUSH_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/brush_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/brush_probe.gd'])]:
        ...
        status='plant_brush_profile_passed'
    finally:
