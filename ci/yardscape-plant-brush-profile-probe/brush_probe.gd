...
func validate_profile(profile:Dictionary,expected_groups:int,expected_cards:int,label:String)->String:
	...
	check(brush.stats.authored_atlas and brush.stats.atlas_id=="northstar-broad-mass-atlas/2",label+" uses the explicit broad-mass Northstar atlas")
...
func run()->void:
	...
	var ash_plan_signature:=plan_signature(ash_profile);var ash_signature:=validate_profile(ash_profile,16,128,"Fan-Tex")
	...
	var palo_plan_signature:=plan_signature(palo_profile);var palo_signature:=validate_profile(palo_profile,14,70,"Palo Verde")
	...
	var report:Dictionary={"run_id":run_id,"passed":failures.is_empty(),"checks":checks,"failures":failures,"pixel_hashes":hashes,"diagnostics":diagnostics,"engine":Engine.get_version_info().string,"adapter":RenderingServer.get_video_adapter_name(),"scope":"fixed-center alpha-scissored brush-card foliage over unchanged profile-driven shared layout, compared with retained dabs","uses_shared_plan_layout":true,"uses_3d_mesh_readback_for_plan":false,"uses_authored_brush_atlas":true,"brush_atlas_id":"northstar-broad-mass-atlas/2","uses_alpha_scissor":true,"uses_alpha_blend":false,"whole_plant_billboard":false,"uses_solid_core":false,"runtime_ai":false,"tablet_performance_tested":false,"artistic_acceptance":"human_review_required"}
