...
def verify():
    ...
    expected={'northstar-fantex-ash-form.tscn','presentation/northstar/spatial_fantex_ash_study.gd','presentation/northstar/foliage/fantex_ash_batched_tree.gd'}
    actual={p.relative_to(ROOT/'project').as_posix() for p in (ROOT/'project').rglob('*') if p.is_file()}
    if actual!=expected:raise ValueError('Unexpected Fan-Tex source expansion')
    for name in actual:
        p=ROOT/'project'/name
        ...
        for ref in re.findall(r'res://([^\s\"\')]+)',text):
            if ref.endswith(('.gd','.gdshader','.gdshaderinc','.tscn')) and not any((base/'project'/ref).is_file() for base in (ROOT,BATCH,VALUE,DABTREE,SPATIAL,CANOPY,GROUP,DAB,COMPACT)):raise ValueError('Unresolved '+ref)
    return True
def run():
    verify()
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'fantex-form'
    if stage.exists():raise FileExistsError('Fresh stage required')
    shutil.copytree(SPATIAL/'project',stage)
    for base in (CANOPY,GROUP,DAB,COMPACT,DABTREE,VALUE,BATCH,ROOT):
    ...
    shutil.copyfile(ROOT/'fantex_probe.gd',stage/'tests/fantex_probe.gd')
    project='''config_version=5
[application]
config/name="Fan-Tex Ash macro form"
run/main_scene="res://northstar-fantex-ash-form.tscn"
[display]
...
'''
    ...
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'fantex-form-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_FANTEX_OUTPUT=str(inside),YARDSCAPE_FANTEX_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/fantex_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/fantex_probe.gd'])]:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Fan-Tex macro-form source boundary verified')
    else:run()
