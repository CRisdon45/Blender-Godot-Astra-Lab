...
## One compact generic foliage group in the existing courtyard.
...
func _build() -> void:
	...
	for child in derived.get_children():
		if child.name=="tree-trunk" or str(child.name).begins_with("crown-"):
			child.visible=false
	group=Group.new()
	group.name="CompactFoliageGroup"
		...
		if layer is CanvasLayer:
			for child in layer.get_children():
				if child is Label and child.text.begins_with("MATERIAL TRANSFER"):
					child.text="COMPACT FOLIAGE GROUP / generic 3D form study / not species or production asset"
