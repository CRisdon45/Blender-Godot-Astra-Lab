...
def run():
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'shoot-group';shutil.copytree(SPATIAL/'project',stage)
    for b in (GROUP,ROOT):
        for p in (b/'project').rglob('*'):
            if p.is_file():
                d=stage/p.relative_to(b/'project');d.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(p,d)
    shutil.copyfile(ROOT/'shoot_probe.gd',stage/'tests/shoot_probe.gd');(stage/'project.godot').write_text('config_version=5\n[application]\nconfig/name="Shoot foliage group comparison"\n[display]\nwindow/size/viewport_width=1280\nwindow/size/viewport_height=900\n[rendering]\nrenderer/rendering_method="gl_compatibility"\n')
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'shoot-group-evidence';out.mkdir(exist_ok=False);rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_SHOOT_OUTPUT=str(inside),YARDSCAPE_SHOOT_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Shoot-group source boundary verified')
    else:run()
