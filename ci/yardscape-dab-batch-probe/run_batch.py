...
def verify():
    spec=importlib.util.spec_from_file_location('value_verify',VALUE/'run_value.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m);m.verify()
    expected={'northstar-batched-dab-value-tree.tscn','presentation/northstar/spatial_batched_dab_value_study.gd','presentation/northstar/foliage/batched_dab_tree.gd','presentation/northstar/foliage/batched_dab_value.gdshader'}
    actual={p.relative_to(ROOT/'project').as_posix() for p in (ROOT/'project').rglob('*') if p.is_file()}
    if actual!=expected:raise ValueError('Unexpected batch source expansion')
    for name in actual:
        p=ROOT/'project'/name
        ...
        for ref in re.findall(r'res://([^\s\"\')]+)',text):
            if ref.endswith(('.gd','.gdshader','.gdshaderinc','.tscn')) and not any((base/'project'/ref).is_file() for base in (ROOT,VALUE,DABTREE,SPATIAL,CANOPY,GROUP,DAB,COMPACT)):raise ValueError('Unresolved '+ref)
        if name.endswith('.gdshader'):
            code=re.sub(r'//[^\n]*','',text)
            if re.search(r'\b(TIME|SCREEN_UV|ALPHA|EMISSION|hint_screen_texture|hint_depth_texture)\b|\bVERTEX\s*=',code):raise ValueError('Unreviewed shader domain')
    return True
def run():
    verify()
    ...
    stage=Path(os.environ['RUNNER_TEMP'])/'dab-batch'
    if stage.exists():raise FileExistsError('Fresh stage required')
    shutil.copytree(SPATIAL/'project',stage)
    for base in (CANOPY,GROUP,DAB,COMPACT,DABTREE,VALUE,ROOT):
    ...
    shutil.copyfile(ROOT/'batch_probe.gd',stage/'tests/batch_probe.gd')
    project='''config_version=5
[application]
config/name="Batched dab hierarchy tree"
run/main_scene="res://northstar-batched-dab-value-tree.tscn"
[display]
...
'''
    ...
    inside=stage/'.local/output';inside.mkdir(parents=True);out=Path(os.environ['RUNNER_TEMP'])/'dab-batch-evidence';out.mkdir(exist_ok=False)
    rid=str(uuid.uuid4());env={k:v for k,v in os.environ.items() if not k.startswith(('PLAN_TRACE_','YARDSCAPE_','GODOT_MCP_'))};env.update(YARDSCAPE_DAB_BATCH_OUTPUT=str(inside),YARDSCAPE_DAB_BATCH_RUN_ID=rid,LIBGL_ALWAYS_SOFTWARE='1',GODOT_SILENCE_ROOT_WARNING='1');status='failed'
    try:
        for logname,cmd in [('preflight.log',[str(engine),'--headless','--path',str(stage),'--script','res://tests/batch_probe.gd','--check-only']),('console.log',['xvfb-run','-a',str(engine),'--path',str(stage),'--audio-driver','Dummy','--script','res://tests/batch_probe.gd'])]:
...
if __name__=='__main__':
    if '--verify-only' in sys.argv:verify();print('Dab batching source boundary verified')
    else:run()
