...
function addTube(out, b, scale = 1) {
  ...
  for (let j = 0; j <= samples; j++) {
    ...
    for (let s = 0; s < sides; s++) {
      ...
      pushVertex(out, q, radial, .52 + b.order * .034 + t * .028);
    }
  }
...
function addVolumetricCluster(out, center, proxy, width, height, rng, variation, tile, layers = 3) {
  ...
  addClusterPlane(out, center.clone().addScaledVector(proxy, rng.signed(width * .025)), n2, width * rng.range(.94, 1.06), height * rng.range(.94, 1.07), rng.range(-Math.PI, Math.PI), (tile + 1 + rng.int(0, 2)) % 4, variation + rng.signed(.006), rng.signed(.10));
  if (layers < 3) return;
  const n3 = proxy.clone().multiplyScalar(.82).addScaledVector(v, rng.next() < .5 ? .28 : -.28).normalize();
  addClusterPlane(out, center.clone().addScaledVector(proxy, rng.signed(width * .018)), n3, width * rng.range(.78, .94), height * rng.range(.82, .98), rng.range(-Math.PI, Math.PI), (tile + 2 + rng.int(0, 1)) % 4, variation + rng.signed(.005), rng.signed(.09));
}
...
function buildGraph(params, recipe) {
  ...
  for (let i = 0; i < s.leaderCount; i++) {
    ...
    for (let j = 0; j < scaffoldCount; j++) {
      ...
      anchors.push({ branch: scaffold, t: .50, weight: .42, role: 'transition' }, { branch: scaffold, t: .66, weight: .58, role: 'transition' });
      for (const at of [.78,.90,.985]) anchors.push({ branch: scaffold, t: at, weight: THREE.MathUtils.lerp(.72, .98, at), role: 'region' });
      ...
      for (let k = 0; k < secondaryCount; k++) {
        ...
        anchors.push({ branch: child, t: .52, weight: .58, role: 'transition' }, { branch: child, t: .68, weight: .68, role: 'transition' });
        for (const at of [.80,.94,.992]) anchors.push({ branch: child, t: at, weight: THREE.MathUtils.lerp(.76, 1.0, at), role: 'region' });
        if (growth.maturity > .70) {
          ...
          anchors.push({ branch: twig, t: .68, weight: .72, role: 'transition' }, { branch: twig, t: .88, weight: .90, role: 'region' }, { branch: twig, t: .992, weight: 1.0, role: 'region' });
        }
      }
    }
  }
...
function regionVariation(anchor, f, graph) {
  const leaderBias = [-.004, 0, .004][anchor.branch.leader % 3];
  const heightBias = THREE.MathUtils.lerp(-.005, .007, THREE.MathUtils.clamp(f.center.y / Math.max(graph.height, .001), 0, 1));
...
function buildFoliage(graph, params, recipe) {
  ...
  graph.anchors.forEach(anchor => {
    ...
    if (f.center.y < graph.height * .29 && anchor.weight < .78) return;
    const yNorm = THREE.MathUtils.clamp(f.center.y / Math.max(graph.height, .001), 0, 1);
    const upperT = THREE.MathUtils.clamp((yNorm - recipe.canopy.upperCrownStart) / Math.max(.001, 1 - recipe.canopy.upperCrownStart), 0, 1);
    const upperScale = THREE.MathUtils.lerp(1, recipe.canopy.upperCrownScale, upperT);
    const baseTone = regionVariation(anchor, f, graph);

    if (anchor.role === 'transition') {
      if (rng.next() < recipe.canopy.transitionCoverage) {
        const width = rng.range(.27,.45) * (.80 + anchor.weight * .28) * THREE.MathUtils.lerp(1,1.16,upperT), height = width * rng.range(.50,.70);
        const p = f.center.clone().addScaledVector(f.u,rng.signed(width*.16)).addScaledVector(f.v,rng.signed(width*.10)).addScaledVector(UP,rng.signed(width*.09));
        addVolumetricCluster(foliage,p,f.proxy,width,height,rng,baseTone-.008,rng.int(0,3),2); masses++; transitions++;
      }
    } else {
      let count = rng.int(2,4); if (upperT > .12) count += recipe.canopy.upperCrownExtraMasses;
      for (let m = 0; m < count; m++) {
        const hierarchy = m === 0 ? 1 : m === 1 ? rng.range(.74,.90) : rng.range(.56,.74);
        const width = rng.range(.38,.72) * (.84 + anchor.weight * .28) * hierarchy * upperScale, height = width * rng.range(.50,.72), angle = m * GOLDEN_ANGLE + rng.signed(.34), scatter = rng.range(.08,.34);
        const p = f.center.clone().addScaledVector(f.u,Math.cos(angle)*width*scatter).addScaledVector(f.v,Math.sin(angle)*width*scatter*.60).addScaledVector(UP,rng.signed(width*.18)).addScaledVector(f.tangent,rng.signed(width*.10));
        if (anchor.t > .90) p.addScaledVector(f.tangent,width*.09);
        if(Math.hypot(p.x,p.z)<avoid&&p.y>graph.height*.31)p.addScaledVector(f.outward,avoid*rng.range(.18,.38));
        const proxy=f.proxy.clone().multiplyScalar(.90).addScaledVector(UP,.06).addScaledVector(f.outward,.04).normalize();
        addVolumetricCluster(foliage,p,proxy,width,height,rng,baseTone+rng.signed(.004),rng.int(0,3),3);masses++;
      }
    }

    const accentChance = anchor.role === 'region' ? recipe.canopy.fineAccentFraction * .58 : recipe.canopy.fineAccentFraction * .22;
    if(rng.next()<accentChance){
      const width=rng.range(.14,.21)*params.sprayScale,height=width*rng.range(.30,.45),p=f.center.clone().addScaledVector(f.u,rng.signed(.08)).addScaledVector(UP,rng.signed(.045));
      addClusterPlane(foliage,p,f.proxy,width,height,rng.range(-Math.PI,Math.PI),rng.int(0,3),baseTone+rng.signed(.006),rng.signed(.09));accents++;
      if(rng.next()<recipe.modules.bloom.probabilityAtTerminal){addClusterPlane(flowers,p.clone().addScaledVector(UP,.020),f.proxy,width*.42,height*.50,rng.range(-Math.PI,Math.PI),rng.int(0,3),.52,rng.signed(.07));bloom++;}
    }
  });
...
export function buildDesertMuseum(params, recipe = DESERT_MUSEUM_PALO_VERDE) {
  ...
  for(const b of graph.branches){addTube(wood,b,1);if(b.order===0)addTube(outline,b,1.024);}
