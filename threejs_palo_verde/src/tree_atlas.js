...
function addTube(out, b, scale = 1) {
  ...
  for (let j = 0; j <= samples; j++) {
    ...
    for (let s = 0; s < sides; s++) {
      ...
      pushVertex(out, q, radial, .53 + b.order * .040 + t * .030);
    }
  }
...
function addClusterPlane(out, center, proxy, width, height, spin, tile, variation, tilt = 0) {
  ...
  const base = out.positions.length / 3, corners = [[-.5,-.5],[.5,-.5],[.5,.5],[-.5,.5]];
...
function addVolumetricCluster(out, center, proxy, width, height, rng, variation, tile, layers = 3) {
  addClusterPlane(out, center, proxy, width, height, rng.range(-Math.PI, Math.PI), tile, variation, rng.signed(.08));
  if (layers < 2) return;
  const { u, v } = frame(proxy);
  const n2 = proxy.clone().multiplyScalar(.80).addScaledVector(u, rng.next() < .5 ? .32 : -.32).normalize();
  addClusterPlane(out, center.clone().addScaledVector(proxy, rng.signed(width * .025)), n2, width * rng.range(.94, 1.06), height * rng.range(.94, 1.07), rng.range(-Math.PI, Math.PI), (tile + 1 + rng.int(0, 2)) % 4, variation + rng.signed(.007), rng.signed(.10));
  if (layers < 3) return;
  const n3 = proxy.clone().multiplyScalar(.82).addScaledVector(v, rng.next() < .5 ? .28 : -.28).normalize();
  addClusterPlane(out, center.clone().addScaledVector(proxy, rng.signed(width * .018)), n3, width * rng.range(.78, .94), height * rng.range(.82, .98), rng.range(-Math.PI, Math.PI), (tile + 2 + rng.int(0, 1)) % 4, variation + rng.signed(.006), rng.signed(.09));
}
...
function growChild(parent, id, order, t, length, sign, rng, radiusScale, upBias, outBias) {
  ...
  return branch(id, order, origin, p1, p2, end, parent.radius0 * radiusScale * (1 - t * .22), parent.radius1 * radiusScale * .70, parent.id, parent.leader);
}
...
function buildGraph(params, recipe) {
  ...
  for (let i = 0; i < s.leaderCount; i++) {
    ...
    const stem = branch(`stem:${i}`, 0, p0, new THREE.Vector3(Math.cos(a) * spread * .03, leaderHeight * .34, Math.sin(a) * spread * .03), new THREE.Vector3(p3.x * .60 + rng.signed(.12), p3.y * .76, p3.z * .60 + rng.signed(.12)), p3, THREE.MathUtils.lerp(.065, .15, growth.maturity), THREE.MathUtils.lerp(.024, .058, growth.maturity), null, i);
    branches.push(stem);
    ...
    for (let j = 0; j < scaffoldCount; j++) {
      ...
      anchors.push({ branch: scaffold, t: .52, weight: .42, role: 'transition' }, { branch: scaffold, t: .67, weight: .56, role: 'transition' });
      for (const at of [.80,.91,.985]) anchors.push({ branch: scaffold, t: at, weight: THREE.MathUtils.lerp(.72, .98, at), role: 'region' });
      ...
      for (let k = 0; k < secondaryCount; k++) {
        ...
        anchors.push({ branch: child, t: .54, weight: .56, role: 'transition' });
        for (const at of [.78,.94,.992]) anchors.push({ branch: child, t: at, weight: THREE.MathUtils.lerp(.74, 1.0, at), role: 'region' });
        if (growth.maturity > .70) {
          ...
          anchors.push({ branch: twig, t: .70, weight: .70, role: 'transition' }, { branch: twig, t: .90, weight: .90, role: 'region' }, { branch: twig, t: .992, weight: 1.0, role: 'region' });
        }
      }
    }
  }
...
function regionVariation(anchor, f, graph) {
  const leaderBias = [-.010, .004, .013][anchor.branch.leader % 3];
  const heightBias = THREE.MathUtils.lerp(-.010, .012, THREE.MathUtils.clamp(f.center.y / Math.max(graph.height, .001), 0, 1));
  return .505 + leaderBias + heightBias;
}

function buildFoliage(graph, params, recipe) {
  const foliage = target(), flowers = target(); let masses = 0, accents = 0, bloom = 0, transitions = 0;
  ...
  graph.anchors.forEach(anchor => {
    ...
    if (f.center.y < graph.height * .30 && anchor.weight < .78) return;
    const baseTone = regionVariation(anchor, f, graph);

    if (anchor.role === 'transition') {
      if (rng.next() < .88) {
        const width = rng.range(.24,.42) * (.78 + anchor.weight * .28), height = width * rng.range(.48,.68), p = f.center.clone().addScaledVector(f.u,rng.signed(width*.18)).addScaledVector(f.v,rng.signed(width*.12)).addScaledVector(UP,rng.signed(width*.10));
        const proxy = f.proxy.clone().multiplyScalar(.90).addScaledVector(f.outward,.10).normalize();
        addVolumetricCluster(foliage,p,proxy,width,height,rng,baseTone-.010,rng.int(0,3),2); masses++; transitions++;
      }
    } else {
      const count = rng.int(2,4);
      for (let m = 0; m < count; m++) {
        const hierarchy = m === 0 ? 1 : m === 1 ? rng.range(.72,.88) : rng.range(.54,.72);
        const width = rng.range(.38,.72) * (.82 + anchor.weight * .28) * hierarchy, height = width * rng.range(.48,.72), angle = m * GOLDEN_ANGLE + rng.signed(.36), scatter = rng.range(.10,.38);
        const p = f.center.clone().addScaledVector(f.u,Math.cos(angle)*width*scatter).addScaledVector(f.v,Math.sin(angle)*width*scatter*.62).addScaledVector(UP,rng.signed(width*.20)).addScaledVector(f.tangent,rng.signed(width*.12));
        if(Math.hypot(p.x,p.z)<avoid&&p.y>graph.height*.32)p.addScaledVector(f.outward,avoid*rng.range(.18,.40));
        const proxy = f.proxy.clone().multiplyScalar(.88).addScaledVector(UP,.07).addScaledVector(f.outward,.05).normalize();
        addVolumetricCluster(foliage,p,proxy,width,height,rng,baseTone+rng.signed(.006),rng.int(0,3),3); masses++;
      }
    }

    const accentChance = anchor.role === 'region' ? recipe.canopy.fineAccentFraction * .64 : recipe.canopy.fineAccentFraction * .28;
    if(rng.next()<accentChance){
      const width=rng.range(.14,.22)*params.sprayScale,height=width*rng.range(.30,.46),p=f.center.clone().addScaledVector(f.u,rng.signed(.09)).addScaledVector(UP,rng.signed(.05));
      addClusterPlane(foliage,p,f.proxy,width,height,rng.range(-Math.PI,Math.PI),rng.int(0,3),baseTone+rng.signed(.008),rng.signed(.10));accents++;
      if(rng.next()<recipe.modules.bloom.probabilityAtTerminal){addClusterPlane(flowers,p.clone().addScaledVector(UP,.022),f.proxy,width*.44,height*.52,rng.range(-Math.PI,Math.PI),rng.int(0,3),.52,rng.signed(.08));bloom++;}
    }
  });
  return { foliage: geom(foliage), flowers: geom(flowers), masses, accents, bloom, transitions };
}
...
export function buildDesertMuseum(params, recipe = DESERT_MUSEUM_PALO_VERDE) {
  ...
  for (const b of graph.branches) { addTube(wood,b,1); if (b.order <= 1) addTube(outline,b,1.042); }
  const crown = buildFoliage(graph,params,recipe), woodG=geom(wood), outlineG=geom(outline), triangles=(woodG.index.count+outlineG.index.count+crown.foliage.index.count+crown.flowers.index.count)/3;
  ...
  return {wood:woodG,outline:outlineG,foliage:crown.foliage,flowers:crown.flowers,flowerCount:crown.bloom,report:{
    schema:'threejs-palo-verde/2',recipeSchema:recipe.schema,recipeId:recipe.id,styleProfile:recipe.styleProfile,seed:params.seed,maturity:params.maturity,targetHeightM:graph.height,targetSpreadM:graph.spread,
    branches:graph.branches.length,structuralBranches:graph.branches.length,microTwigs:0,foliageMasses:crown.masses,fineSprays:crown.accents,transitionMasses:crown.transitions,foliageBrushes:crown.masses+crown.accents,flowerBrushes:crown.bloom,
    triangles,triangleTarget:recipe.lod0Budget.triangleTarget,triangleHardCap:recipe.lod0Budget.triangleHardCap,targetMet:triangles<=recipe.lod0Budget.triangleTarget,lod:0,calendarCalibrated:false,
    foliageRepresentation:'painted foliage atlas on fixed 3D quadmesh with bent proxy normals; region-composed canopy'
  }};
}
